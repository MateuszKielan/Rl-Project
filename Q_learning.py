...
class QLearningAgentModified:
    ...
    def train(self, total_episodes = 2000, max_steps = 100, display_interval = 20):
        ...
        for episode in range(total_episodes):
            ...
            #reseting visit counts for this episode
            episode_visits = np.zeros(self.env.maze.shape)
            
            while not done and step < max_steps:
                step += 1
                
                #converting state to 2D coordinates (position)
                pos = (state[0], state[1])
                
                #adding the visit to tracking
                episode_visits[pos] += 1
                
                #taking action based on epsilon-greedy strategy
...
#END OF MAIN Q-LEARNING ALGORITHM SECITON
